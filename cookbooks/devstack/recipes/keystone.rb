...
if create_monasca_agent_user:
    monasca_agent_user = key.users.create(name='monasca-agent', password='password', email='monasca-agent@mon.com', tenant_id=tenant_id, enabled=True)

#create role for monasca agent
if create_monasca_agent_user:
    role = key.roles.create('monasca-agent')
    key.roles.add_user_role(monasca_agent_user, role, tenant_id);
EOH
end
