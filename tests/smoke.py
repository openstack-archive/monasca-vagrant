...
config_dict = smoke_configs.test_config["default"]
...
def set_config(config_name):
    global config_dict
    try:
        config_dict = smoke_configs.test_config[config_name]
        print('Using '+config_name+' Configuration')
    except KeyError:
        print('Could not find config "'+config_name+'"')
        sys.exit(1)
...
def smoke_test(mail_host, metric_host):
    notification_name = config_dict['notification']['name']
    notification_email_addr = config_dict['notification']['email_addr']
    alarm_definition_name = config_dict['alarm']['name']
    metric_name = config_dict['metric']['name']
    metric_dimensions = config_dict['metric']['dimensions']
    ...
    expression = config_dict['alarm']['expression']
    description = config_dict['alarm']['description']
    ...
    if alarm_id is None:
        ...
        if received_num_metrics == initial_num_metrics:
            msg = ('Did not receive any %s metrics while waiting' %
                   metric_name + str(metric_dimensions))
        else:
            delta = received_num_metrics - initial_num_metrics
            msg = ('Received %d %s metrics while waiting' %
                   (delta, metric_name))
        return False, msg
    ...
    if not check_notifications(alarm_id, states[1:]):
        msg = 'Could not find any notifications for alarm %s' % alarm_id
        return False, msg
...
def find_processes():
    ...
    process_missing = []
    process_list = config_dict['system_vars']['expected_processes']
...
def main():
    ...
    set_config(cmd_args.config)

    mail_host = config_dict['system_vars']['mail_host']
    metric_host = config_dict['system_vars']['metric_host']
    ...
    if find_processes():
        print()
        print('*****BEGIN TEST*****')
        complete, msg = smoke_test(mail_host, metric_host)
        if not complete:
...
if __name__ == "__main__":
    result = main()
    cleanup(config_dict['notification']['name'], config_dict['alarm']['name'])
    sys.exit(result)
